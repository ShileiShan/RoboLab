...
launch_replay baseline "$baseline_run_id" "$baseline_kp" "$baseline_kd" "$baseline_armature" "$baseline_friction"
launch_replay candidate "$candidate_run_id" "$candidate_kp" "$candidate_kd" "$candidate_armature" "$candidate_friction"
...
python3 analysis/build_piper_joint_tracking_report.py \
  ...
  --baseline-label "Baseline (friction=${baseline_friction}, armature=${baseline_armature}, kp=${baseline_kp}, kd=${baseline_kd})" \
