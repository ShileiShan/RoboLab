...
"""Replay a real-robot Piper joint trace by hard-setting robot state each frame."""
...
parser = argparse.ArgumentParser(
    description="Run a strict Piper joint-state replay from a real-robot qpos trace.",
    allow_abbrev=False,
)
...
if args_cli.num_envs != 1:
    parser.error("Joint-state replay currently requires --num-envs 1.")
if trace.steps < 2:
    parser.error("Joint-state replay requires at least 2 reference states.")
...
def _build_output_paths(args: argparse.Namespace) -> tuple[str, str]:
    output_folder_name = args.output_folder_name or (get_timestamp() + "_piper_joint_state_replay")
...
def _write_reference_pose(env, target: np.ndarray) -> None:
    ...
    left_arm = torch.as_tensor(target[0:6], device=env.device).unsqueeze(0)
    right_arm = torch.as_tensor(target[6:12], device=env.device).unsqueeze(0)
    left_gripper = float(target[12])
    right_gripper = float(target[13])
...
def _write_pose_and_observe(env, target: np.ndarray):
    _write_reference_pose(env, target)
    for _ in range(3):
        env.sim.render()
        env.scene.update(dt=0.0)
    return env.observation_manager.compute(update_history=False)
...
def _build_metrics(
    *,
    ...
    summary = {
        ...
        "hdf5_path": hdf5_path,
        "replay_mode": "joint_state_hard_set",
...
def main() -> None:
    ...
    env_cfg.instruction = "Replay the recorded Piper joint state."
    ...
    try:
        ...
        obs = _write_pose_and_observe(env, trace.env_targets[0])
        ...
        if args_cli.video_mode == "viewport":
            ...
            video_recorder = ComparisonVideoRecorder.create(
                ...
                video_stem="joint_state_replay_viewport",
                video_fps=video_fps,
        ...
        if trace.steps > 1:
            initial_action = torch.as_tensor(trace.env_targets[0], device=env.device, dtype=torch.float32)
            obs, _, _, _, _ = env.step(initial_action.unsqueeze(0))
            if env.all_terminated:
                raise RuntimeError("State replay terminated immediately after the initial frame.")

        for step_idx in range(1, trace.steps):
            obs = _write_pose_and_observe(env, trace.env_targets[step_idx])
            actual_state[step_idx] = _extract_env_state(obs)
            if video_recorder is not None:
                video_recorder.write_frame(obs, env_id=0)
            if step_idx != trace.steps - 1:
                ...
                if env.all_terminated:
                    raise RuntimeError(
                        f"State replay terminated early at step {step_idx} before the reference finished."
                    )

        time_s = np.arange(trace.steps, dtype=np.float32) / np.float32(trace.control_hz)
        hdf5_path = Path(scene_output_dir) / "run_0.hdf5"
        summary_path = Path(scene_output_dir) / "joint_state_replay_summary.json"
        timeseries_path = Path(scene_output_dir) / "joint_state_replay_timeseries.npz"
        ...
        print(
            f"[RoboLab] Joint-state replay complete: {trace.steps} reference states, "
            f"{replay_steps} control steps at {trace.control_hz:g} Hz."
        )
        print(f"[RoboLab] Output dir: {output_dir}")
        print(f"[RoboLab] Replay summary: {summary_path}")
        print(f"[RoboLab] Replay timeseries: {timeseries_path}")
